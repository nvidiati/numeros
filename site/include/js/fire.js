 ...
 //New York Ayer Tarde
  Firebase.ref("lotteryNumbers").orderByChild("filter3").equalTo(ayer + "NEW YORKt").on('child_added', function(snapshot){
    ...
    if (respuesta.first.length > 0) {
      //Si hay numeros de ayer para mostrar
      masLoteryAyer();

    }else{
      //Si no hay numeros de ayer para mostrar
    }

  });
  
  function masLoteryAyer(){
    /////////////// FUNCTION masLoteryAyer/ /////////////
 ...
 //New York hoy Tarde
  Firebase.ref("lotteryNumbers").orderByChild("filter3").equalTo(hoy + "NEW YORKt").on('child_added', function(snapshot){
    ...
    if (respuesta.first.length > 0) {
      //Si hay numeros de hoy para mostrar
      masLoteryHoy();

    }else{
      //Si no hay numeros de hoy para mostrar
    }
  });

  function masLoteryHoy(){
    /////////////// FUNCTION masLoteryAyer/ /////////////
   //Loteria Real hoy Tarde
